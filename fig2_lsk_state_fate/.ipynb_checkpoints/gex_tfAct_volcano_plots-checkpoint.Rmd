...
setwd("~/wustl_drive/phd/MGIseq/lineage_priming/feature-enrichment-statecells/")
...
#   gene.expr <- gene.expr[order(gene.expr$colors_curr, decreasing = TRUE),]
#   p1 <- ggscatter(gene.expr,x='delta',y='p.adj.log', size=0.5, color = "colors_curr", palette = c("#0e4fad","grey"),label=rownames(gene.expr),repel = TRUE, font.label = c(7),label.select = rownames(gene.expr[rownames(gene.expr) %in% gene_markers[[i]],]),xlab = gsub("_","/",names(gene_markers)[[i]]),show.legend.text=FALSE, ylab = FALSE) + geom_hline(yintercept=1.3, linetype="dashed", color = "black", size=0.3) + geom_vline(xintercept=0.5, linetype="dashed", color = "black", size=0.3) + geom_vline(xintercept=-0.5, linetype="dashed", color = "black", size=0.3) + theme_pubr(base_size = 6.5, base_family = "Helvetica") + theme(legend.position="none",axis.title.y=element_blank())
#   plot_list[[i]] <- p1
# }
...
```{r}
keyvals <- ifelse(
    rownames(gene.expr) %in% gene_markers[['Baso_Eos_Mast']], 'royalblue','grey')


EnhancedVolcano(gene.expr,
    lab = rownames(gene.expr),
    x = 'delta', selectLab = gene_markers[['Baso_Eos_Mast']],
    y = 'p.adj', cutoffLineType = 'blank',drawConnectors = TRUE)

  p2 <- EnhancedVolcano(gene.expr,
    lab = rownames(gene.expr),
    x = 'delta',
    y = 'p.adj',
    selectLab = gene_markers[['Baso_Eos_Mast']],
...
p1 <- EnhancedVolcano(gene.expr,
  ...
  borderWidth = 1,
  caption="",
  borderColour = 'black') + ggpubr::theme_pubr(base_size = 10) + ylab("") + xlab("") + theme(legend.position = "none")

p1

ggsave("./220208/tst.pdf", p1, height = 4, width = 3)
```
